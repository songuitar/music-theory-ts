...
export class HarmonicSequence {
  ...
  static fromStringChords(chords: string[], key: string) {
    ...
    return new HarmonicSequence(
      HarmonicSequence.asDigitized(
        keyChord,
        chords.map((chord: string) => Chord.fromString(chord))
      ),
      keyChord
    )
  }
  ...
  static findKeyCandidate(chords: string[], preferType = ChordType.minor): string | null {
    const overlappingNodes = CircleOfFifths.getHarmonyOverlapMap(chords).filter(({ node }) => {
      return (
        chords.includes(node.majorChord.toString()) && chords.includes(node.minorChord.toString())
      )
    })

    const entry =
      overlappingNodes.find(({ node }) => {
        return (
          chords.includes(node.prev.majorChord.toString()) ||
    ...
    return preferType === ChordType.minor
      ? entry.node.minorChord.toString()
      : entry.node.majorChord.toString()
  }
  ...
  asChords(): Chord[] {
    ...
    return this.digitized.map(offset =>
      HarmonicSequence.resolveChordByStringOffset(offset, keyScale)
    )
  }
